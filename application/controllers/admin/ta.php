...
class Ta extends CI_Controller
{
	function __construct(){
		...
		$this->load->model(array('m_jurusan','m_konsentrasi','m_level','m_mhs','m_tapel'));
		$this->output->nocache();
	...
	function index(){
		...
		if($level=="10" or $level=="11" or $level=="12" or $level=="13"){
			...
			$data['akademik'] = $this->m_tapel->get_group_tapel();
		}else{
			$data['konsentrasi'] = $this->m_konsentrasi->get_kon_jur();
			$data['akademik'] = $this->m_tapel->get_all_tapel();
		}
		$this->template->load2('template-admin','page/admin/premahasiswaakhir',$data);
	}
	...
	function detail(){
		...
		if ($akad==''){
			echo "<script>alert('Tahun Akademik belum dipilih');</script>";
			echo "<script>history.go(-1);</script>";
		}
		if ($smest==''){
			echo "<script>alert('Semester belum dipilih');</script>";
			echo "<script>history.go(-1);</script>";
		}
		
		if(!empty($akad) && !empty($smest)) {
			if($smest=='1'){
				$sm ='GASAL';
			}else if ($smest=='2'){
				$sm ='GENAP';
			...
			$data['tapel'] = $this->m_tapel->get_rekap($akad, $sm);
			$this->template->load2('template-admin','page/admin/mhsakhir', $data);
		}
	}
