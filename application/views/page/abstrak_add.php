					...
					<div class="box box-primary">
						...
						<?php
							if($this->uri->segment(2)=="addabs"){
								$files=$file;
								$act ="add";
								$lap_id="";
							}elseif($this->uri->segment(2)=="editabs"){
								foreach($proposal->result() as $key){
									$files = $key->lap_file;
								}
								$act ="edit";
								$lap_id=$this->uri->segment(3);
							}elseif($this->uri->segment(2)=="submitabs"){
								$files=$file;
								$act =$link;
								$lap_id=$id;
							}
						?>
							 <form role="form" class="form-horizontal" enctype="multipart/form-data" action="<?=site_url('proposal/submitabs');?>" method="POST" >
